...
- **Добавлен Sharding**
  ...
  - Каждый шард защищён отдельным `RwLock` для снижения блокировок и повышения параллелизма в чтении.
  - Конфигурируемое количество шардов (по умолчанию `num_cpus * 2`).
...
### Изменено
  ...
  - Интегрирована поддержка шардированного индекса в `src/engine/persistent.rs`:
  ...
  - Обновлено логирование операций в AOF с учётом шардирования.
