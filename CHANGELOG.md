...
- **database/dict**
  - Добавлены comprehensive unit-тесты для Entry API (`tests/dict/entry.rs`):
    ...
    - Проверка корректной работы `or_insert_with_key`.
    - Проверка `or_default` для vacant и occupied entry.
    - Проверка `and_modify` для occupied и vacant entry.
    - Реализован тест word-count сценария через Entry API.
    - Проверка методов `OccupiedEntry`:
      - `get`, `get_mut`, `into_mut`
      - `insert` с возвратом старого значения.
    - Подтверждена корректность поведения Entry API без двойного поиска и без лишних аллокаций.
    - Тесты гарантируют полную совместимость семантики с `std::collections::HashMap::entry`.

- **database/dict**
  - Реализован полный Entry API для эффективных операций вставки и модификации без двойного поиска:
    - Добавлен enum `Entry<'a, K, V, S>` с вариантами:
      - `OccupiedEntry` — для существующего ключа;
      - `VacantEntry` — для отсутствующего ключа.
    - Добавлены структуры:
      - `OccupiedEntry<'a, K, V>` — предоставляет доступ к существующему элементу без повторного поиска;
      - `VacantEntry<'a, K, V, S>` — позволяет эффективно вставлять новый элемент напрямую в слот таблицы.
  - Реализованы методы `OccupiedEntry`:
    - `key()` — получение ссылки на ключ;
    - `get()` — получение ссылки на значение;
    - `get_mut()` — получение изменяемой ссылки на значение;
    - `into_mut()` — получение изменяемой ссылки с временем жизни словаря;
    - `insert(val)` — замена значения с возвратом старого;
    - `remove()` — удаление элемента из словаря с возвратом значения.
  - Реализованы методы `VacantEntry`:
    - `key()` — получение ссылки на ключ;
    - `into_key()` — извлечение ключа;
    - `insert(val)` — вставка нового элемента напрямую в слот без повторного поиска.
  - Реализованы convenience-методы `Entry`:
    - `or_insert(default)`
    - `or_insert_with(f)`
    ...
    - `and_modify(f)`
    - `key()`
  - Добавлен метод `Dict::entry(key)` для выполнения insert-or-modify операций с одним lookup.
  - Устранён паттерн двойного поиска (`get` + `insert`), что улучшает производительность.
  - Вставка выполняется напрямую в bucket slot через `&mut Option<Box<DictNode>>`, без повторного хеширования и traversal.
  - API полностью совместим по семантике с `std::collections::HashMap::entry`, обеспечивая привычный и ergonomic интерфейс.

- **database/dict**
  - Добавлены тесты для адаптивного рехеширования:
    - Проверка, что все ключи остаются доступными во время инкрементального рехеша.
    - Проверка, что рехеширование завершается после достаточного числа операций.
    - Тест удаления и повторной вставки во время активного рехеша.
  - Добавлены тесты для автоматического сжатия (shrink):
    - Ёмкость уменьшается корректно после массового удаления элементов.
    - Автоматическое сжатие не срабатывает при умеренном удалении.
    - Оставшиеся элементы сохраняются после сжатия.
  - Добавлены тесты для `shrink_to_fit`:
    - Проверка, что `shrink_to_fit` уменьшает ёмкость до минимально необходимой.
    - Проверка, что все элементы остаются доступными после сжатия.
  - Общие улучшения покрытия:
    - Длинные цепочки удаляются итеративно без переполнения стека.
    - Проверка работы с разными хешерами (`ahash`, `FxBuildHasher`).
    - Проверка корректного поведения `get_mut`, `clear`, `reserve` и итераций в различных условиях.
...
* **database/dict**
  ...
  - Добавлена поддержка инкрементального рехеширования во всех мутационных операциях:
    ...
    - `remove`
    - `get_mut`
  - Добавлены интеграционные тесты `tests/dict_correctness_tests.rs`:
    - 18 тестов, покрывающих edge cases и корректность рехеширования;
    - пустой словарь и один элемент;
    - большие collision chains (до 50k элементов);
    - корректность работы при активном рехешировании;
    - корректность итератора при наличии двух таблиц;
...
* **database/skiplist**
  - Добавлена потокобезопасная реализация SkipList:
    ...
    - Независимые shard'ы позволяют выполнять операции параллельно без глобальной блокировки.
    - Atomic счётчики (`AtomicUsize`) для отслеживания общего количества элементов и метрик операций.
    - Добавлены метрики производительности: распределение данных по shard'ам, load balance score, per-shard statistics.
  - Добавлены тесты конкурентного доступа:
    - concurrent inserts, reads, removes;
    - mixed workloads с множественными потоками;
    - stress tests для проверки корректности и thread safety.
  - Добавлены benchmarks concurrent и sharded SkipList:
    - Insert: до **2.73× быстрее**
    - Search: до **3.53× быстрее**
    - Remove: до **2.78× быстрее**
    - Iterate: до **2.81× быстрее**
    - Throughput: до **23M ops/sec**
    - Traversal throughput: до **66M elements/sec**
  - Оптимизирована однопоточная версия `skiplist_base`:
    - улучшена cache locality и traversal efficiency;
...
* **tests**
  ...
  - Интеграционный тест направлений соседних geohash-ячееек (`test_geohash_neighbor_directions`).
  - Интеграционный тест иерархии geohash, включая parent/child отношения (`test_geohash_hierarchy`).
  - Тест производительности для больших наборов данных (10 000 точек), включая глубину R-дерева и распределение корзин geohash (`test_large_dataset_performance`).
  - Интеграционный тест для проверки согласованности запросов между только R-деревом и R-деревом с geohash (`test_query_consistency`).
...
* **geo**
  - добавлен новый модуль `geohash` (`geo/geo_hash.rs`), реализующий:
    ...
    - нахождение соседних ячеек (`neighbor`, `all_neighbors`);
    - родительских и дочерних ячеек (`parent`, `children`);
    - генерацию диапазонов геохешей для заданного bbox (`geohash_ranges_for_bbox`);
...
- **engine**
  - в `memory.rs` исправлена логика заимствования в функции `geo_radius`:
    используется `get_mut` вместо `get`, так как вызов `GeoSet::radius` и последующий доступ к данным требуют изменяемой ссылки.
    Исправлено нарушение правил заимствования Rust и потенциальная ошибка работы с памятью.

- **geo**
  - обновлён модуль `geo_base.rs` для интеграции `geohash`:
    - `GeoSet` теперь поддерживает индексацию гео-объектов с использованием `geohash`;
    - добавлены методы `get_geohash`, `radius_with_geohash`, `bbox_query` и `geohash_stats`;
    - улучшена функция добавления и обновления точек (`add`) с пересборкой индексов;
    - добавлены тесты для проверки интеграции геохеша с `GeoSet`, поиска по радиусу, ближайших соседей и корректности статистики;
    - исправлена фильтрация по координатам и обработка краевых случаев.

- **examples**
  - Переименован пакет в `Cargo.toml` с `examples` на `zumic_examples`.
    Это исправление связано с тем, что имя examples конфликтовало с внутренними директориями сборки Cargo, из-за чего Dependabot не мог обновлять зависимости и анализировать workspace.

- **auth/session**
  - в `manager.rs` изменина логика работы ф-ии `create_session_from_token`:
    - устранена возможность race condition между read & write
    - проверка лимита и создание сессии теперь выполняются атомарно
    - гарантируется соблюдение лимита при высокой конкурентности
  - Изменена логика работы ф-ии `validate_session_full`:
    - валидация и обновление активности выполняются под одним `write-lock`
    - используется метод `with_session` для атомарной проверки и модификации сессии;
    - устранены потенциальные race condition при конкурентной валидации.
  - Изменена логика работы ф-ии `get_username`:
    - устранены лишние `drop` и повторные `write-lock`
    - проверка истечения сессии и чтение username выполняются атомрано под одним `write-lock`
    - повышена корректность работы при конкурентном доступе
...
- **database/dict**
  - Исправлена сигнатура `get`: теперь использует `&self` вместо `&mut self`, что восстанавливает корректный read-only контракт API.
  - Удалён вызов `rehash_step()` из `get`, чтобы избежать скрытой мутации во время чтения.
  - Рекурсивный `remove_from_chain` заменён на итеративную реализацию, предотвращающую stack overflow при длинных collision chains.
  - Исправлен `expand_if_needed`: при `size == 0` теперь корректно инициализируется `INITIAL_SIZE`, предотвращая невозможность расширения.
  - Исправлены edge cases при:
    - вставке в пустой словарь;
    - удалении последнего элемента;
    - работе во время активного рехеширования.
