...
/// Пул для повторного использования `Arc<str>` для одинаковых
/// имён каналов.
///
/// Видимость: внутри текущего крейта (crate-private), недоступен
/// снаружи.
static CHANNEL_INTERN: Lazy<DashMap<String, Arc<str>>> = Lazy::new(DashMap::new);

/// Возвращает интернированный `Arc<str>` для заданного имени
/// канала.
/// При первом вызове с новым именем создаёт `Arc<str>` и сохраняет
/// его в пуле.
#[inline(always)]
...
mod tests {
    ...
    /// Тест проверяет, что первый вызов создаёт `Arc<str>` с
    /// правильным содержимым,
    /// а повторный вызов возвращает тот же самый объект (без
    /// копирования).
    #[test]
    ...
    /// Тест проверяет, что разные имена каналов дают разные
    /// `Arc<str>`.
    #[test]
    ...
    /// Тест проверяет, что `String` и строковый литерал с
    /// одинаковым содержимым интернируются в один и тот же
    /// `Arc<str>`.
    #[test]
    ...
    /// с одним и тем же ключом из разных потоков возвращают
