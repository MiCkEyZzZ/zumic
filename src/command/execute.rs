...
use super::{
    ...
    HmGetCommand, IncrByCommand, IncrByFloatCommand, IncrCommand, LLenCommand, LPopCommand,
    LPushCommand, LRangeCommand, LRemCommand, LSetCommand, MGetCommand, MSetCommand, PfAddCommand,
    PfCountCommand, PfMergeCommand, RPopCommand, RPushCommand, RenameCommand, RenameNxCommand,
    SAddCommand, SCardCommand, SDiffCommand, SInterCommand, SIsMemberCommand, SMembersCommand,
    SPopCommand, SRandMemberCommand, SRemCommand, SUnionCommand, SetBitCommand, SetCommand,
    SetFloatCommand, SetNxCommand, StrLenCommand, XAckCommand, XAddCommand, XDelCommand,
...
/// Генерирует диспетчеризацию `execute` для всех вариантов [`Command`].
///
/// Каждый вариант делегирует вызов `cmd.execute(store)` внутренней структуре,
/// что позволяет избежать дублирования при добавлении новых команд.
macro_rules! dispatch_execute {
    ($self:expr, $store:expr, $($variant:ident),* $(,)?) => {
        match $self {
            $(Command::$variant(cmd) => cmd.execute($store),)*
        }
    };
}
...
impl Command {
    ...
    pub fn key(&self) -> Option<&[u8]> {
        match self {
            ...
            Command::HIncrBy(cmd) => Some(cmd.key.as_bytes()),
            Command::HIncrByFloat(cmd) => Some(cmd.key.as_bytes()),
...
impl CommandExecute for Command {
    fn execute(
        ...
        let result = dispatch_execute!(
            self, store,
            Set, Get, Del, Exists, SetNx, MSet, MGet, Rename,
            Renamenx, Flushdb, Strlen, Append, GetRange,
            Incr, Incrby, Decr, Decrby, Incrbyfloat, Decrbyfloat,
            Setfloat,
            HSet, HGet, HmGet, HDel, HExists, HLen, HKeys, HVals,
            HGetall, HRandField, HIncrBy, HIncrByFloat,
            HPfAdd, HPfCount, HPfMerge,
            SAdd, SRem, SCard, SMembers, SIsmember, SRandMember,
            SPop, SInter, SUnion, SDiff,
            ZAdd, ZRem, ZRange, ZScore, ZCard, ZRevrange, ZRank,
            ZRevRank, ZCount, ZIncrBy,
            LSet, LRem, LPush, RPush, LPop, RPop, LLen, LRange,
            Auth,
            ...
            PUnsubscribe, PubSub,
            Ping, Echo, DbSize, Info, Time, Select, Save, BgSave,
            Shutdown,
            XAdd, XRead, XRange, XRevRange, XLen, XDel, XTrim,
            XGroupCreate, XAck,
            TsCreate, TsAdd, TsGet, TsRange, TsDel,
        );
