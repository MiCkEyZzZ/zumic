...
mod tests {
    ...
    /// Тест проверяет, что директива `requirepass` корректно
    /// парсится и сохраняется в поле `requirepass`, а список
    /// пользователей остаётся пустым.
    #[test]
    ...
    /// Тест проверяет корректность парсинга одного пользователя
    /// с параметрами: включён (`on`), без пароля (`nopass`), с
    /// доступом ко всем ключам (`~*`) и всеми правами (`+@all`).
    #[test]
    ...
    /// Тест проверяет парсинг конфигурации с несколькими
    /// пользователями.
    /// Удостоверяется, что `requirepass` установлен и оба
    /// пользователя корректно разобраны.
    #[test]
    ...
    /// Тест проверяет поведение при попытке парсинга пользователя
    /// в некорректном формате: ожидание ошибки из-за недостаточного
    ...
    /// Тест проверяет, что при наличии неизвестной директивы в
    /// строке пользователя возникает ошибка парсинга.
    #[test]
