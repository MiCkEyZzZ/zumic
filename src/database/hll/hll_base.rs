...
impl Hll {
    ...
    /// 2. Делит хэш на `index` (старшие 14 бит) и `rho` (количество ведущих
    ///    нулей в оставшихся битах + 1).
    /// 3. Обновляет регистр под номером `index`, если вычисленное `rho` больше
    ///    текущего.
    pub fn add(
        &mut self,
        value: &[u8],
    ...
    /// 3. При малых оценках (≤ 2.5·m) и наличии нулевых регистров — использует
    ///    коррекцию Лог-Лог.
