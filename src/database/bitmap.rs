//! Структура и операции для эффективной работы с битовыми массивами (Bitmap) в Zumic.
//!
//! Модуль реализует структуру [`Bitmap`] для хранения и обработки битовых данных с поддержкой:
//! - установки и получения битов по индексу,
//! - подсчёта установленных битов в диапазоне,
//! - побитовых логических операций (`AND`, `OR`, `XOR`, `NOT`) между битовыми массивами.
//!
//! Используется, например, для реализации команд: `SETBIT`, `GETBIT`, `BITCOUNT`, `BITOP` и др.
//!
//! # Пример использования
//! ```rust
//! use zumic::database::Bitmap;
//! let mut bmp = Bitmap::new();
//! bmp.set_bit(10, true);
//! let is_set = bmp.get_bit(10);
//! ```
...
/// Lookup-таблица для подсчёта количества установленных битов в байтах от 0 до 255.
...
/// Динамический битовый массив с поддержкой побитовых операций и подсчёта битов.
///
/// Используется для хранения и обработки битов, поддерживает эффективные побитовые операции и подсчёт установленных битов.
...
impl Bitmap {
    ...
    /// Создаёт `Bitmap` с заданной длиной в битах, все биты инициализируются нулями.
    ///
    /// # Аргументы
    ...
    ///
    /// При необходимости массив расширяется автоматически.
    ///
    /// # Возвращает
    /// `true`, если бит был установлен до изменения, иначе `false`.
    pub fn set_bit(
        &mut self,
        bit_offset: usize,
        value: bool,
    ...
    /// Возвращает значение бита по смещению `bit_offset`.
    ///
    /// Если бит выходит за границы массива, возвращает `false`.
    pub fn get_bit(
        &self,
        bit_offset: usize,
    ) -> bool {
    ...
    /// Подсчитывает количество установленных (`true`) битов в диапазоне `[start, end)`.
    ///
    ...
    /// Возвращает длину битового массива в битах (кратно 8).
...
mod tests {
    ...
    /// Тест проверяет подсчёт установленных битов в заданном
    /// диапазоне.
    #[test]
    fn test_bitcount() {
        let mut bitmap = Bitmap::new();
    ...
    /// Тест проверяет побитовые операции `AND`, `OR`, `XOR`
    /// между двумя Bitmap.
    #[test]
