...
//!
//! Этот модуль реализует эффективную структуру данных для
//! хранения строк, которая использует стек (stack) для
//! коротких строк и кучу (heap) для длинных, обеспечивая
//! компактность и высокую производительность. Структура
//! автоматически переключается между двумя режимами хранения
...
impl Sds {
    ...
    /// Создаёт Sds из вектора байт, выбирая стек или кучу в зависимости
    /// от размера.
    #[inline(always)]
...
mod tests {
    ...
    /// Тест проверяет создание строки, которая помещается в стековое
    /// представление.
    #[test]
    ...
    /// Тест проверяет создание строки, которая превышает лимит стека и
    /// переходит в кучу.
    #[test]
    ...
    /// Тест проверяет добавление одного байта, помещающегося в стековое
    /// представление.
    #[test]
    ...
    /// Тест проверяет переход от стека к куче при добавлении байта, превышающего
    /// лимит.
    #[test]
    ...
    /// Тест проверяет, что усечение переводит строку из кучи обратно в стек
    /// при возможности.
    #[test]
    ...
    /// Тест корректного преобразования строки в `String`, если это допустимая
    /// UTF-8.
    #[test]
