...
//! алгоритмы для хранения, поиска и вычисления расстояний между точками на сфере.
//! Поддерживаются операции добавления, поиска по радиусу, вычисления расстояний,
//! а также кодирование/декодирование координат в geohash.
//!
//! Используется для реализации команд: `GEOADD`, `GEODIST`, `GEORADIUS`, `GEOPOS` и др.
//!
//! # Пример использования
//! ```rust
//! use zumic::database::{GeoSet, GeoPoint};
...
impl GeoSet {
    ...
    /// Возвращает всех членов в радиусе `radius_m` метров от точки (`lon`, `lat`).
    pub fn radius(
        &self,
        lon: f64,
        lat: f64,
...
/// Вычисляет расстояние между двумя точками на сфере (метры, формула гаверсина).
///
/// # Аргументы
/// * `p1` — первая точка.
/// * `p2` — вторая точка.
...
/// Грубая оценка максимального дельта по score для радиуса (используется для оптимизации поиска).
///
/// # Аргументы
