//! Структуры и алгоритмы для работы с географическими данными (GEO) в Zumic.
//!
//! Модуль реализует структуры [`GeoPoint`], [`GeoEntry`], [`GeoSet`] и
//! алгоритмы для хранения, поиска и вычисления расстояний между точками на
//! сфере. Поддерживаются операции добавления, поиска по радиусу, вычисления
//! расстояний, а также кодирование/декодирование координат в geohash.
//!
//! Используется для реализации команд: `GEOADD`, `GEODIST`, `GEORADIUS`,
//! `GEOPOS` и др.
//!
//! # Пример использования
//! ```rust
//! use zumic::database::{GeoPoint, GeoSet};
//! let mut geo = GeoSet::new();
//! geo.add("city".into(), 37.62, 55.75);
//! let pos = geo.get("city");
//! ```
...
/// Множество географических точек с быстрым поиском по радиусу и имени.
...
///
/// # Аргументы
/// * `v` — значение (координата).
/// * `min` — минимальная граница диапазона (будет изменена).
/// * `max` — максимальная граница диапазона (будет изменена).
///
/// # Возвращает
/// 1, если v >= середины диапазона, иначе 0.
fn refine_bit(
    v: f64,
    min: &mut f64,
    max: &mut f64,
...
///
/// # Аргументы
/// * `hash` — 52-битный geohash.
///
/// # Возвращает
/// Кортеж (долгота, широта) центра ячейки.
#[allow(dead_code)]
...
/// Грубая оценка максимального дельта по score для радиуса (используется для
/// оптимизации поиска).
///
/// # Аргументы
/// * `_radius_m` — радиус в метрах.
///
/// # Возвращает
/// Приближённое значение дельта по score.
#[allow(dead_code)]
...
mod tests {
    ...
    /// - второй вызов уточняет нижнюю половину и возвращает 0, оставляя min =
    ///   50.0 и устанавливая max = 75.0.
    #[test]
    fn test_refine_bit() {
        let mut min = 0.0;
        let mut max = 100.0;
    ...
    /// - `decode_geohash_bits` восстанавливает координаты с погрешностью <
    ///   0.001.
    #[test]
    ...
    /// расстояние между (0°,0°) и (0°,1°) должно быть примерно 111 195 м ± 100
    /// м.
    #[test]
    ...
    /// - возвращает только те точки, что находятся в пределах заданного
    ///   радиуса;
