...
//!
//! Модуль предоставляет безопасную и ограниченную среду
//! для выполнения Lua-скриптов внутри базы данных Zumic.
//! Используется для расширяемости, написания пользовательских
//! процедур, атомарных операций и скриптовых команд.
...
///
/// Представляет различные типы ошибок, которые могут возникнуть при выполнении Lua скриптов
/// в безопасной среде Zumic DB.
#[derive(Debug)]
...
/// Все лимиты применяются для предотвращения DoS-атак и обеспечения стабильности системы.
///
/// # Поля
///
...
///
/// Предоставляет изолированную среду для выполнения Lua-скриптов с автоматическим
/// контролем ресурсов, преобразованием типов и обработкой ошибок.
///
/// # Безопасность
///
...
/// # Поддерживаемые типы
///
/// Движок автоматически конвертирует между Lua-типами и внутренними типами Zumic:
/// - `string` ↔ `Sds`
/// - `number` ↔ `i64`/`f64`
/// - `boolean` ↔ `bool`
        ...
        match value {
            ...
            Value::SStream(ss) => {
                ...
                for (i, entry) in ss.iter().enumerate() {
                    ...
                    lua_table.set(i + 1, entry_table)?; // Lua-массивы начинаются с 1
                }
...
///
/// Позволяет использовать LuaExecutionError в контексте стандартной обработки ошибок Rust.
...
mod tests {
    ...
    ///
    /// Проверяет, что бесконечные циклы корректно прерываются по лимиту инструкций.
    #[test]
