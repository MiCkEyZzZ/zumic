...
/// Данные — это ассоциативный массив (ключ-значение) с произвольными
/// значениями.
...
mod tests {
    ...
    /// Вспомогательная функция для создания записи с одним ключом и значением
    /// Int.
    fn make_entry(
        key: &str,
        val: i64,
    ...
    /// Тест проверяет методы new, is_empty и len, который корректно создаёт
    /// новый поток пустым, и что методы `is_empty` и `len` корректно
    /// отражают состояние.
    #[test]
