...
//!
//! Содержит утилиты для решения, когда применять сжатие, а также функции
//! для компрессии и декомпрессии.

use std::io;
use zstd::stream::{decode_all, encode_all};

/// Минимальный размер в байтах, при котором стоит применять сжатие.
/// Если длина блока данных меньше этой константы, сжатие не выполняется.
const MIN_COMPRESSION_SIZE: usize = 64;

/// Проверяет, нужно ли пытаться сжать блок данных заданного размера.
///
/// # Аргументы
///
...
///
/// Использует уровень компрессии 3, который обеспечивает баланс между
...
///
/// Попытка декомпрессии всегда выполняется, даже если размер блока мал.
///
/// # Аргументы
///
...
mod tests {
    ...
    /// Тест проверяет, что сжатие и последующая декомпрессия маленького блока
    /// возвращают исходные данные.
    #[test]
