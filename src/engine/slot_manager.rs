...
///
/// Поля:
/// - `operations_per_second` — приблизительные операции/секунду по каждому шарду;
...
/// Хранит прогресс миграции и список ключей, если он заполняется во время сканирования шарда.
#[derive(Debug)]
...
/// - `slot_map_version` — атомарная версия map'ы (инкрементируется при изменениях).
...
/// Небольшая реализация consistent-hash кольца для альтернативной маршрутизации.
/// Хранит map hash -> shard и отсортированный массив хешей для быстрых обходов.
#[derive(Debug)]
...
impl ConsistentHashRing {
    /// Создание кольца с `shard_count` узлами и `virtual_nodes_per_shard` виртуальными нодами.
    /// Использует `DefaultHasher` (SipHash или реализация std).
    pub fn new(
        shard_count: usize,
        virtual_nodes_per_shard: usize,
    ) -> Self {
    ...
    ///
    /// Примечание: это упрощённая реализация; для больших колец имеет смысл избегать
    /// выделения/сортировки при каждом изменении, а также использовать более быстрый hasher.
    pub fn get_shard(
        &self,
        key: &str,
    ) -> ShardId {
...
impl SlotManager {
    /// Создание SlotManager с `shard_count` шардов и начальным round-robin распределением
    /// 16384 слотов между ними.
    ///
    /// Производительность:
    /// - `record_operation` — lock-free (атомики), подходит для высокочастотных путей.
    /// - Операции управления slot_map используют `RwLock` (корректно для редких изменений).
    ...
    /// если ключ содержит `{...}`, то хешируется только содержимое внутри фигурных скобок.
    ...
    ///
    /// Важно: это не обновляет агрегированный `load_metrics` напрямую — он должен быть
    /// заполнен периодическим агрегатором (background worker), чтобы не тратить CPU в hot-path.
    pub fn record_operation(
        ...
        // ПРИМЕЧАНИЕ: сознательно не обновляем HashMap в критическом/горячем пути.
        // Сбор и агрегация данных в load_metrics выполняются периодически фоновым заданием.
    }

    /// Везвращает `true`, если по текущим метрикам стоит инициировать ребаланс.
    ///
    /// Правила:
    /// - Если присутствуют агрегированные метрики (`load_metrics.operations_per_second`), они имеют приоритет.
    /// - Иначе используется быстрый fallback на атомики `shard_ops`.
    /// - Также учитывается интервал `rebalance_interval` — если недавно делался ребаланс,
    ///   пропускаем (кроме случая, когда у нас есть явные агрегированные метрики).
    pub fn should_rebalance(&self) -> bool {
        ...
        // Если есть агрегированные операции в load_metrics, используем их (тесты и background-агрегатор).
        {
            let metrics = self.load_metrics.read().unwrap();
    ...
    ///
    /// Алгоритм:
    /// 1. Сначала пытается использовать агрегированные `load_metrics` (если они непустые).
    /// 2. Если агрегированных метрик нет — использует атомики `shard_ops` и `slot_access`.
    ///
    /// Примечание: реальное копирование данных между шардами не реализовано здесь —
    /// этот модуль только планирует и отслеживает состояние слотов и задач миграции.
    pub fn trigger_rebalance(&self) -> Vec<(SlotId, ShardId, ShardId)> {
        // Предпочитаем агрегированные метрики, если они есть (оператор / тесты / агрегатор)
    ...
    /// Создаёт задачу миграции: переводит слот в состояние `Migrating` и регистрирует задачу.
    ///
...
mod tests {
    ...
    /// когда агрегированные метрики (`load_metrics.operations_per_second`) содержат
    /// сильно неравномерную нагрузку (1000 vs 100), метод должен вернуть `true`.
    #[test]
    ...
    /// - `start_slot_migration` переводит слот в состояние `Migrating` и регистрирует задачу;
    /// - `complete_slot_migration` завершает миграцию и переводит слот в `Stable(to_shard)`.
    #[test]
