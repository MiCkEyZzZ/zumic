...
impl AofLog {
    ...
    /// Открывает (или создаёт) AOF-файл по указанному пути.
    ///
    /// Проверяет или записывает магический заголовок и настраивает буферизацию.
    /// Если политика `EverySec`, запускается фоновый поток сброса.
    ///
    /// # Возвращает:
    /// - `Ok(AofLog)` при успешном открытии и инициализации файла
    /// - `Err(io::Error)` при некорректном заголовке или ошибке открытия
    ...
    /// Создаёт AOF-файл с параметрами по умолчанию.
    ///
    /// # Возвращает:
    /// - `Ok(AofLog)` при успешном создании
    /// - `Err(io::Error)` при ошибке открытия
    pub fn open_default<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        Self::open(path, SyncPolicy::EverySec, CorruptionPolicy::Log)
    }

    /// Добавляет в AOF журнал команду `SET` с ключом и значением.
    ///
    /// Формат записи: `[AofOp::Set][checksum][key_len][key][val_len][val]`.
    /// В зависимости от политики синхронизации выполняется немедленный или
    /// отложенный flush.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешной записи
    /// - `Err(io::Error)` при ошибке записи или сброса буфера
    pub fn append_set(
        &mut self,
        key: &[u8],
        value: &[u8],
    ...
    /// Добавляет в AOF журнал команду `DEL` с ключом.
    ///
    /// Формат записи (AOF2): `[AofOp::Del][checksum][key_len][key]`.
    /// В зависимости от политики синхронизации выполняется немедленный или
    /// отложенный flush.
    ///
    /// # Возвращает:
    ...
    ///
    /// Для каждой операции `SET` или `DEL` вызывается переданный callback
    /// `f(op, key, val)`. Для `DEL` значение передаётся как `None`.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешном воспроизведении всех операций
    /// - `Err(io::Error)` при ошибке чтения файла или некорректных данных
    pub fn replay<F>(
        &mut self,
        f: F,
    ) -> io::Result<()>
    ...
    /// Компактирует AOF-журнал, записывая только актуальные ключи.
    ///
    /// Используется временный файл, затем атомарно заменяет оригинальный.
    /// Всегда применяется формат AOF2 с checksum.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешной переписи
    /// - `Err(io::Error)` при ошибке записи или замены файла
    pub fn rewrite<I, P>(
        &mut self,
        path: P,
        live: I,
    ...
    /// Возвращает текущие метрики AOF-журнала.
    ///
    /// Полезно для мониторинга и отладки.
    ///
    /// # Возвращает:
    /// - `AofMetrics` с актуальными значениями всех счётчиков
    ...
    ///
    /// # Возвращает:
    /// - `Ok(IntegrityStats)` с результатами проверки
    /// - `Err(io::Error)` при ошибке чтения файла или повреждённых данных
    ...
    /// Выполняет ремонт AOF-файла с указанной стратегией.
    ///
    /// В зависимости от `mode` может пропускать, строго проверять или пытаться
    /// восстановить повреждённые записи. При необходимости перезаписывает файл.
    ///
    /// # Возвращает:
    /// - `Ok(RepairResult)` с информацией о восстановлении, модификациях и
    ///   сообщениях
    /// - `Err(io::Error)` при строгом режиме и критической ошибке чтения
    ...
    /// Записывает `u32` в формате big-endian.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешной записи
    /// - `Err(io::Error)` при ошибке записи
    #[inline]
    fn write_32<W: Write>(
        w: &mut W,
        v: u32,
    ...
    /// Безопасно читает `u32` в формате big-endian из буфера.
    ///
    /// # Возвращает:
    /// - `Ok(u32)` при успешном чтении
    /// - `Err(io::Error)` если в буфере недостаточно байт
    #[inline]
    fn read_u32(
        buf: &[u8],
        pos: &mut usize,
    ...
    /// Выполняет flush в соответствии с текущей политикой синхронизации.
    ///
    /// Для `Always` выполняется немедленный flush и обновляются метрики.
    /// Для `EverySec` и `No` flush не выполняется.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешном flush или если flush не требуется
    /// - `Err(io::Error)` при ошибке записи буфера
    ...
    /// Обновляет метрики: время flush, количество и ошибки.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешном flush
    /// - `Err(io::Error)` при ошибке записи буфера
    ...
    /// Адаптивно подстраивает размер батча для flush при политике `Always`.
    ///
    /// - Если активности не было > 5 секунд, уменьшает batch в 2 раза (минимум
    ///   4).
    /// - При активности < 1 секунды назад возможно увеличение (не реализовано).
    ...
    /// Воспроизводит AOF-журнал в старом формате AOF1 (без checksum).
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешном воспроизведении
    /// - `Err(io::Error)` при нарушении целостности в режиме `Strict`
    ...
    /// Воспроизводит одну запись в формате AOF1.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешной обработке записи
    /// - `Err(io::Error)` при ошибке чтения ключа/значения или некорректной
    ///   операции
    ...
    /// Воспроизводит AOF-журнал в новом формате AOF2 (с checksum).
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешном воспроизведении
    /// - `Err(io::Error)` при нарушении целостности в режиме `Strict`
    ...
    /// Воспроизводит одну запись в формате AOF2 с проверкой checksum.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешной обработке записи
    /// - `Err(io::Error)` при ошибке валидации, усечённых данных или
    ...
    /// Определяет конец текущей записи в формате AOF2.
    ///
    /// # Возвращает:
    /// - `Ok(pos)` — индекс конца записи в буфере (для SET включает ключ и
    ///   значение)
    /// - `Ok(buf.len())` — если запись усечена или данных недостаточно
    fn find_record_end(
        &self,
        buf: &[u8],
        start: usize,
    ...
    /// Парсит валидную запись AOF2 и вызывает callback с разобранными данными.
    ///
    /// # Возвращает:
    /// - `Ok(())` при успешном парсинге
    /// - `Err(io::Error)` при усечённых данных или переполнении позиции
    ...
    /// Находит следующую потенциально валидную запись в буфере после ошибки.
    ///
    /// Используется при replay с CorruptionPolicy::Skip/Log для пропуска
    /// повреждённых записей.
    ///
    /// # Возвращает:
    /// - `Some(usize)` — индекс начала следующей потенциальной записи
    /// - `None` — если не найдено валидных кандидатов
...
////////////////////////////////////////////////////////////////////////////////
// Общие реализации трейтов для AofOp, AofLog
////////////////////////////////////////////////////////////////////////////////
